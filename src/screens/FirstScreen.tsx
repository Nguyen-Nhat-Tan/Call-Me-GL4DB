...
export default function FirstScreen({ navigation }: { navigation: any }) {
  ...
  const sendNoti = ({ item }: { item: any }) => {
    fetch('https://3545-2402-800-61cd-5de7-e974-bcbe-869c-8d17.ngrok.io/send-noti', {
      method: 'post',
      headers: {
  ...
  const callbacks = {
    EndCall: () => { setVideoCall(false), firestore().collection('users').doc(authID).update({ calling: false }) }
  };
  return (
    videoCall === false ?
